    ...
    <main dir={language === 'ar' ? 'rtl' : 'ltr'} className="correction-page">
      ...
      <section className="correction-shell">
        ...
        <form className="correction-form" onSubmit={handleSubmit}>
          ...
          <label>{text.service}<NativeSelect value={listingId} disabled={listings.length === 0} onChange={(event) => setListingId(event.target.value)}>{listings.length === 0 ? <NativeSelectOption value="">{text.noServices}</NativeSelectOption> : listings.map((listing) => <NativeSelectOption key={listing.id} value={listing.id}>{listing.name}</NativeSelectOption>)}</NativeSelect></label>
          ...
          <Button className="submit-correction" type="submit" disabled={!listingId || !privacyConfirmed || message.trim().length < 10 || state === 'sending'}>{state === 'sending' ? text.sending : text.submit}</Button>
